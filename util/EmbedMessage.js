...
class EmbedMessage extends Discord.MessageEmbed {
    ...
    static automatiqueMessage(type, client, author, title, ajout){
        let numero = 1;
        while(message[type][numero]){
            numero = numero +1
        }
        let random = parseInt(Math.random() * ((numero) - (1)) + (1));
        let description = message[type][random]
        return new EmbedMessage(client, {
            title: title,
