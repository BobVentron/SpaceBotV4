...
module.exports = {
  ...
  async execute (client, message, args) {
    if (await AgenceManager.targetExists(message.author.id)) {
      ...
      if (data.cash >= client.get("market/renommage").price) {
        Agence.findOneAndUpdate({ userID: message.author.id }, { cash: data.cash - client.get("market/renommage").price }).catch(err => console.log(err));
        let entite = args[0]
        let oldName = args[1]
        let newName = args[2]
        ...
        return EmbedMessage.showError(client, "**Payload - Erreur**", `Il vous manque ${client.get("market/renommage").price - data.cash}$ pour pouvoir effectuer cette action.`)
      }
    } else {
