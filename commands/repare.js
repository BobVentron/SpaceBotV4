...
module.exports = {
  ...
  async execute (client, message, args) {
    if (await AgenceManager.targetExists(message.author.id)) {
      ...
      if (data.sabotage >= 1) {
        if (data.cash >= client.get("market/reparage").price && data.science >= client.get("market/reparage").pds) {
          cashUpdated = parseInt(data.cash) - client.get("market/reparage").price;
          scienceUpdated = parseInt(data.science) - client.get("market/reparage").pds;
