...
module.exports = {
  name: "join",
  use: "**/join [USA / Russie]**",
  description: "Permet de rejoindre une des deux équipes dans la Conquête spatiale.",
  options: [
    {
      name: "equipe",
        ...
        if (args[0] == "USA" || args[0] == "Russie") {
          ...
          if (now.getTime() < checkJoinDate.getTime()) {
            ...
            if (data.USAJoin > 0 || data.RussieJoin > 0) {
              ...
              const joinCost = client.get("market/guerrefroide").price
              if (data.cash >= joinCost) {
                ...
                return EmbedMessage.showError(client, "**Join - Erreur**", "Vous devez avoir au moins 800$ sur vous pour pouvoir rejoindre la Conquête spatiale !")
              }
            }
          } else {
            return EmbedMessage.showError(client, "**Join - Erreur**", "Impossible de rejoindre la Conquête spatiale à 2 jours de la fin ...")
          }
        } else {
